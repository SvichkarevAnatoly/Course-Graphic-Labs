/****************************************************************************
** Meta object code from reading C++ file 'circle.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.0.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "circle.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'circle.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.0.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_Circle_t {
    QByteArrayData data[11];
    char stringdata[54];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    offsetof(qt_meta_stringdata_Circle_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData) \
    )
static const qt_meta_stringdata_Circle_t qt_meta_stringdata_Circle = {
    {
QT_MOC_LITERAL(0, 0, 6),
QT_MOC_LITERAL(1, 7, 7),
QT_MOC_LITERAL(2, 15, 0),
QT_MOC_LITERAL(3, 16, 1),
QT_MOC_LITERAL(4, 18, 7),
QT_MOC_LITERAL(5, 26, 1),
QT_MOC_LITERAL(6, 28, 7),
QT_MOC_LITERAL(7, 36, 1),
QT_MOC_LITERAL(8, 38, 4),
QT_MOC_LITERAL(9, 43, 4),
QT_MOC_LITERAL(10, 48, 4)
    },
    "Circle\0changeX\0\0x\0changeY\0y\0changeR\0"
    "r\0setX\0setY\0setR\0"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_Circle[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   44,    2, 0x05,
       4,    1,   47,    2, 0x05,
       6,    1,   50,    2, 0x05,

 // slots: name, argc, parameters, tag, flags
       8,    1,   53,    2, 0x0a,
       9,    1,   56,    2, 0x0a,
      10,    1,   59,    2, 0x0a,

 // signals: parameters
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void, QMetaType::Int,    5,
    QMetaType::Void, QMetaType::Int,    7,

 // slots: parameters
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void, QMetaType::Int,    5,
    QMetaType::Void, QMetaType::Int,    7,

       0        // eod
};

void Circle::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Circle *_t = static_cast<Circle *>(_o);
        switch (_id) {
        case 0: _t->changeX((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 1: _t->changeY((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 2: _t->changeR((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 3: _t->setX((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 4: _t->setY((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 5: _t->setR((*reinterpret_cast< int(*)>(_a[1]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (Circle::*_t)(int );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&Circle::changeX)) {
                *result = 0;
            }
        }
        {
            typedef void (Circle::*_t)(int );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&Circle::changeY)) {
                *result = 1;
            }
        }
        {
            typedef void (Circle::*_t)(int );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&Circle::changeR)) {
                *result = 2;
            }
        }
    }
}

const QMetaObject Circle::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_Circle.data,
      qt_meta_data_Circle,  qt_static_metacall, 0, 0}
};


const QMetaObject *Circle::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *Circle::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Circle.stringdata))
        return static_cast<void*>(const_cast< Circle*>(this));
    return QObject::qt_metacast(_clname);
}

int Circle::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 6)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 6;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 6)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 6;
    }
    return _id;
}

// SIGNAL 0
void Circle::changeX(int _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void Circle::changeY(int _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void Circle::changeR(int _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}
QT_END_MOC_NAMESPACE
